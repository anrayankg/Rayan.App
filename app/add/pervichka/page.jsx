    ...
    async function loadExisting() {
     try {
      ...
      if (l) {
        ...
        setDeliveryYear(l.delivery_year != null ? String(l.delivery_year) : "");
        setDeliveryQuarter(l.delivery_quarter != null ? String(l.delivery_quarter) : "");
        setDocs(l.documents || []);
  ...
  async function handleSaveDraft() {
    ...
    try {
      const payload = {
        ...
        delivery_year: !delivered ? numOrNull(deliveryYear) : null,
        delivery_quarter: !delivered ? numOrNull(deliveryQuarter) : null,
  ...
  return (
    <div className="app-shell" style={{ paddingBottom: 40 }}>
      ...
      <Accordion title="КВАРТИРА" defaultOpen={true}>
        ...
        <MiniField label="Где взять планировку, если нет на руках" value={extra.planirovkaGde || ""} onChange={setEx("planirovkaGde")} />
      ...
      <Accordion title="УСЛОВИЯ ПОКУПКИ">
        <MiniField label="Сумма в договоре купли-продажи" value={extra.summaDkp || ""} onChange={setEx("summaDkp")} />
        <MiniField label="Сумма в ДДУ" value={extra.summaDdu || ""} onChange={setEx("summaDdu")} />
        <MiniField label="Фактическая сумма сделки" value={extra.summaFakt || ""} onChange={setEx("summaFakt")} />
        <MiniYesNo label="Ипотека через банк" value={extra.ipoteka || ""} onChange={setEx("ipoteka")} />
        <MiniYesNo label="Рассрочка от застройщика" value={extra.rassrochkaZastroy || ""} onChange={setEx("rassrochkaZastroy")} />
        {extra.rassrochkaZastroy === "Да" && (
          <MiniField label="Условия рассрочки от застройщика" value={extra.rassrochkaUsloviya || ""} onChange={setEx("rassrochkaUsloviya")} />
        )}
        <MiniField label="Цена за м²" value={extra.cenaM2 || ""} onChange={setEx("cenaM2")} />
        <MiniField label="Коммунальные платежи перед сделкой" value={extra.komUslugi || ""} onChange={setEx("komUslugi")} />
      </Accordion>
...
export default function PervichkaPage() {
  return (
